...
- **Transactional email** — a branded "Your photo is ready" email fires via Resend automatically when a restoration completes. No-op for anonymous users and when `RESEND_API_KEY` is not set.
- **Era estimation** — automatically estimates the decade a photo was taken using OpenRouter (Gemini Flash 1.5); result shown on the restore page. Best-effort: any failure returns null silently.
